...
var shell: SHELL_TYPE = SHELL_TYPE.UNKNOWN

func _ready() -> void:
	var output: Array[String] = []
	match OS.get_name():
		"Windows":
			if (OS.execute("where", ["pwsh"], output) == OK
				and output.size() > 0):
				shell = SHELL_TYPE.WIN_PS7
			elif (OS.execute("where", ["powershell"], output) == OK
				and output.size() > 0):
				shell = SHELL_TYPE.WIN_PS
			else:
				shell = SHELL_TYPE.WIN_CMD
		"Linux", "FreeBSD", "NetBSD", "OpenBSD", "BSD":
			# 优先尝试根据桌面环境判断终端
			var desktop_env: String = OS.get_environment("XDG_CURRENT_DESKTOP").to_lower()
			if desktop_env.contains("kde"):
				if (OS.execute("which", ["konsole"], output) == OK
					and output.size() > 0):
					shell = SHELL_TYPE.LINUX_KDE
			elif desktop_env.contains("gnome"):
				if (OS.execute("which", ["gnome-terminal"], output) == OK
					and output.size() > 0):
					shell = SHELL_TYPE.LINUX_GNOME
			elif desktop_env.contains("xfce"):
				if (OS.execute("which", ["xfce4-terminal"], output) == OK
					and output.size() > 0):
					shell = SHELL_TYPE.LINUX_XFCE
			elif desktop_env.contains("lxqt"):
				if (OS.execute("which", ["qterminal"], output) == OK
					and output.size() > 0):
					shell = SHELL_TYPE.LINUX_LXQT
			# 回滚到通用判断
			if shell == SHELL_TYPE.UNKNOWN:
				if (OS.execute("which", ["konsole"], output) == OK
					and output.size() > 0):
					shell = SHELL_TYPE.LINUX_KDE
				elif (OS.execute("which", ["gnome-terminal"], output) == OK
					and output.size() > 0):
					shell = SHELL_TYPE.LINUX_GNOME
				elif (OS.execute("which", ["xfce4-terminal"], output) == OK
					and output.size() > 0):
					shell = SHELL_TYPE.LINUX_XFCE
				elif (OS.execute("which", ["qterminal"], output) == OK
					and output.size() > 0):
					shell = SHELL_TYPE.LINUX_LXQT
		"macOS":
...
func open_terminal_to_dir(dir_path: String) -> void:
	...
	match shell:
		...
		SHELL_TYPE.LINUX_KDE:
			OS.create_process("konsole", ["--workdir", dir_path], true)
		SHELL_TYPE.LINUX_GNOME:
			OS.create_process("gnome-terminal", ["--working-directory=%s" % dir_path], true)
		SHELL_TYPE.LINUX_XFCE:
